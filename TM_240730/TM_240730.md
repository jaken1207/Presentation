...
[Next.js Docs](https://ja.next-community-docs.dev/docs/app-router/building-your-application/)
...
<!-- header: _Next.js Documents Summary_ > _Routing_-->

# Data Fetching

<!-- header: _Next.js Documents Summary_ > _Data Fetching_-->

- データのフェッチ、キャッシュ、再検証
- Server Actions とミューテーション
- データのフェッチのパターンとベストプラクティス
...
- getData 関数内で次の操作が行われます
  1. fetch の呼び出し
     ※fetch 関数は Promise を返すため、await はその Promise が解決（または拒否）されるのを待つ。
  2. レスポンスの検証
...
<!-- header: _Next.js Documents Summary_ > _Rendering_-->

# Cashing

<!-- header: _Next.js Documents Summary_ > _Cashing_-->
